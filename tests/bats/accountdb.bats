...
@test "account import (db)" {
	run $GETH_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $GOPATH/src/github.com/etclabscore/go-ethereum/cmd/geth/testdata/guswallet.json <<< $'foo\n'
	echo "$output"
...
@test "account import pass mismatch (db)" {
	run $GETH_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $GOPATH/src/github.com/etclabscore/go-ethereum/cmd/geth/testdata/guswallet.json <<< $'wrong\n'
	echo "$output"
