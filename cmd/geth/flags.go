...
var (
	...
	ChainIdentityFlag = cli.StringFlag{
		Name:  "chain",
		Usage: `Chain identifier (default='mainnet', test='mordor') or path to JSON chain configuration file (e.g., './path/to/chain.json').`,
		Value: core.DefaultConfigMainnet.Identity,
	}
	NetworkIdFlag = cli.IntFlag{
		Name:  "network-id, networkid",
		Usage: "Network identifier (integer: 1=Homestead, 7=Mordor)",
		Value: eth.NetworkId,
	}
	TestNetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "[Use: --chain=mordor] Mordor network: pre-configured classic test network with replay protection",
	}
